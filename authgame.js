...
googleSignIn.addEventListener('click', e=>{
    ...
    firebase.auth().signInWithRedirect(provedor).then(userCredential =>{
      return firebase.firestore().collection('users').doc(userCredential.user.uid).set({    
        bio: TXTbio.value,
        local: TXTlocal.value,
        vendas: 0
      }); // oq vai no form de cadastro tem a bio e o local preenchidos o que vai no inicio eles tem string vazia
    });
    e.preventDefault();
  });
